...
const CaseStudyDHSecheron = () => {
  const highlights = [
    "Google Workspace deployment across all manufacturing plants",
    "Unified platform for production and R&D teams",
    "Technical documentation for welding specifications",
    "Quality management with ISO documentation",
    "Dealer network portal for distributors",
    "Google Vault for audit trails and compliance"
  ];
  ...
  const challenges = [
    ...
    "Managing complex supply chain with raw materials and finished goods"
  ];

  const solutions = [
    "Google Workspace connecting plants, R&D, and sales teams",
    "Structured storage for certifications and quality documents",
    "Real-time communication tools for multi-plant coordination"
  ];

  const results = [
    { label: "Years of Excellence", value: "55+" },
    { label: "Plant Connectivity", value: "100%" },
    { label: "Faster Documentation", value: "40%" },
    { label: "IT Cost Savings", value: "30%" }
  ];

  return (
    <div className="min-h-screen bg-white">
      <Helmet>
        <link rel="canonical" href="https://www.shivaami.com/case-studies/dh-secheron" />
      </Helmet>
      <Header />
        ...
        <div className="w-full px-8 lg:px-16 xl:px-24 pt-32 pb-16">
          ...
          >
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-4 leading-tight">
              D&H Sécheron<br />
              <span className="text-[#38B6FF]">Industrial Manufacturing Excellence</span>
            </h1>
            
            <p className="text-base sm:text-lg text-white/90 leading-relaxed max-w-2xl">
              Major manufacturer of welding consumables since 1966, powering India's industrial infrastructure with modern collaboration tools.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="bg-gray-50 border-b border-gray-200">
        <div className="w-full px-8 lg:px-16 xl:px-24">
          <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-gray-200">
            {[
              { label: "Industry", value: "Welding Consumables" },
              { label: "Legacy", value: "Since 1966" },
              { label: "Reach", value: "Pan-India & Export" },
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
              <motion.div className="flex items-center gap-2 mb-4" initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.4 }}>
                <motion.div className="w-10 h-10 rounded-lg bg-amber-100 flex items-center justify-center" whileHover={{ scale: 1.1, rotate: 5 }} transition={{ type: "spring", stiffness: 300 }}>
              ...
              <div className="space-y-3">
                {challenges.map((challenge, index) => (
                  <motion.div key={index} className="p-3 bg-white rounded-lg border-l-4 border-amber-400 shadow-sm hover:shadow-md hover:border-amber-500 transition-all cursor-default" initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.15, duration: 0.4 }} whileHover={{ x: 5, scale: 1.02 }}>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.2 }}>
              <motion.div className="flex items-center gap-2 mb-4" initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.2 }}>
                <motion.div className="w-10 h-10 rounded-lg bg-green-100 flex items-center justify-center" whileHover={{ scale: 1.1, rotate: -5 }} transition={{ type: "spring", stiffness: 300 }}>
              ...
              <div className="space-y-3">
                {solutions.map((solution, index) => (
                  <motion.div key={index} className="p-3 bg-white rounded-lg border-l-4 border-green-500 shadow-sm hover:shadow-md hover:border-green-600 transition-all cursor-default" initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 + index * 0.15, duration: 0.4 }} whileHover={{ x: -5, scale: 1.02 }}>
        ...
        <div className="w-full px-8 lg:px-16 xl:px-24">
          <div className="grid lg:grid-cols-2 gap-8 items-center">
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="relative rounded-2xl overflow-hidden shadow-xl">
              <img src={googleWorkspaceBanner} alt="D&H Sécheron Manufacturing Excellence" className="w-full h-64 lg:h-80 object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-[#0C4594]/60 to-transparent" />
              <div className="absolute bottom-4 left-4 right-4">
                <p className="text-white font-medium text-sm">Connected manufacturing since 1966</p>
              </div>
            </motion.div>
          </div>
        </div>
...
export default CaseStudyDHSecheron;